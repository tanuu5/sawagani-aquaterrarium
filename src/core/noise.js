// CPU 側のノイズ（地形・岩・配置の生成用）
// Simplex noise: Stefan Gustavson の実装をベースにシード対応
...
export class Noise {
  ...
  // 尾根状（岩の稜線用）
  ridged3(x, y, z, oct = 4, lac = 2.1, gain = 0.5) {
    let a = 1, f = 1, s = 0, n = 0, w = 1;
    for (let i = 0; i < oct; i++) {
      let v = 1 - Math.abs(this.noise3(x * f, y * f, z * f));
      v *= v;
      v *= w;
...
// Worley（セル）ノイズ 2D: 最近点距離 F1 と セルID
const _h = (x, y, s) => {
...
export function worley2(x, y, seed = 0) {
  ...
  for (let j = -1; j <= 1; j++) {
    for (let i = -1; i <= 1; i++) {
      ...
      const px = cx + _h(cx, cy, seed);
      const py = cy + _h(cx, cy, seed + 17);
      const dx = px - x, dy = py - y;
      const d = dx * dx + dy * dy;
      if (d < best) { best = d; id = _h(cx, cy, seed + 31); }
    }
  }
  return { d: Math.sqrt(best), id };
}

export const hash2 = _h;
